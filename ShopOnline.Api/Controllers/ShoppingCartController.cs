    ...
    public class ShoppingCartController : ControllerBase
    {
        ...
        public async Task<ActionResult<CartItemDto>> GetItem (int id)
        {
            try
            {
                ...
                var product = await _productRepository.GetProductById(cartItem.Id);
        ...
        public async Task<ActionResult<CartItemDto>> PostItem([FromBody] CartItemToAddDto cartItemToAddDto)
        {
            try
            {
                ...
                if (product == null)
                {
                    throw new Exception($"Something went wrong when attempting to retrive a product (productId:{cartItemToAddDto.ProductId})");
                }
        ...
        public async Task<ActionResult<CartItemDto>> DeleteItem(int id)
        {
            try
            {
                ...
                if (product == null)
                {
                    throw new Exception($"Something went wrong when attempting to retrive a product (productId:{cartItem.ProductId})");
                }
